...
For a TPM-style plugin that does not require Swift at runtime, see [tmux Plugin](tmux-plugin.md). The plugin uses `bash`, `curl`, and `jq` directly and is better suited for shared tmux setups.
