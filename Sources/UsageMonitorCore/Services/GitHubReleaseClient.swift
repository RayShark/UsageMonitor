...
package protocol GitHubReleaseRequestLoading {
    func data(for request: URLRequest) async throws -> (Data, URLResponse)
}

extension URLSession: GitHubReleaseRequestLoading {}

package final class GitHubReleaseClient: GitHubReleaseProviding {
    private let requestLoader: GitHubReleaseRequestLoading
    private let decoder = JSONDecoder()

    package init(requestLoader: GitHubReleaseRequestLoading = URLSession.shared) {
        self.requestLoader = requestLoader
    }

    package func fetchReleases() async throws -> [GitHubRelease] {
