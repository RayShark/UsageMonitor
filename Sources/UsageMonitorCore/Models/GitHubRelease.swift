...
package protocol GitHubReleaseProviding {
    func fetchReleases() async throws -> [GitHubRelease]
}

package struct GitHubReleaseAsset: Equatable, Decodable {
    package let name: String
    package let browserDownloadURL: URL

    package init(name: String, browserDownloadURL: URL) {
        self.name = name
...
package struct GitHubRelease: Equatable, Decodable {
    ...
    package let assets: [GitHubReleaseAsset]
    ...
    package var version: AppVersion? {
        AppVersion.parse(tagName)
    }

    package var dmgAsset: GitHubReleaseAsset? {
        assets.first { $0.name == "UsageMonitor.dmg" }
    }

    package var checksumAsset: GitHubReleaseAsset? {
...
package enum GitHubReleaseClientError: Error, Equatable {
